...
class SemanticPOSS(torch_data.Dataset):
    def __init__(self, mode, grid=[64, 64, 16]):
        self.name = 'SemanticKITTI'
        self.dataset_path = '/home/chx/Work/SemanticPOSS_dataset_grid_0.01/sequences'
    ...
    def __getitem__(self, item):
        # selected_pc, selected_labels, random_pc, random_labels, selected_idx, cloud_ind, pos = self.spatially_regular_gen(item, self.data_list)
        # return selected_pc, selected_labels, random_pc, random_labels, selected_idx, cloud_ind, pos
        return self.spatially_regular_gen(item, self.data_list)
    ...
    def spatially_regular_gen(self, item, data_list):
        ...
        selected_pc, selected_labels, random_pc, random_labels, selected_idx= self.crop_pc(pc, labels, tree, pick_idx)

        return selected_pc, selected_labels, random_pc, random_labels, selected_idx, np.array([cloud_ind], dtype=np.int32)
    ...
    def crop_pc(self, points, labels, search_tree, pick_idx):
        ...
        return select_points, select_labels, random_points, random_label, select_idx
    ...
    def tf_map(self, batch_pc, batch_label, random_pc, random_labels, batch_pc_idx, batch_cloud_idx):
        features = batch_pc
        input_points = []
        input_neighbors = []
        input_pools = []
        ...
        random_features = random_pc
        random_input_points = []
        random_input_neighbors = []
        random_input_pools = []
        random_input_up_samples = []

        for i in range(cfg.num_layers):
            neighbour_idx = DP.knn_search(random_pc, random_pc, cfg.k_n)
            sub_points = random_pc[:, :random_pc.shape[1] // cfg.sub_sampling_ratio[i], :]

            pool_i = neighbour_idx[:, :random_pc.shape[1] // cfg.sub_sampling_ratio[i], :]
            up_i = DP.knn_search(sub_points, random_pc, 1)

            random_input_points.append(random_pc)
            random_input_neighbors.append(neighbour_idx)
            random_input_pools.append(pool_i)
            random_input_up_samples.append(up_i)
            random_pc = sub_points

        random_input_list = random_input_points + random_input_neighbors + random_input_pools + random_input_up_samples
        random_input_list += [random_features, random_labels]

        return input_list, random_input_list


    def collate_fn(self, batch):

        selected_pc, selected_labels, random_pc, random_labels, selected_idx, cloud_ind = [], [], [], [], [], []
        idx = []
        for i in range(len(batch)):
            selected_pc.append(batch[i][0])
            selected_labels.append(batch[i][1])
            random_pc.append(batch[i][2])
            random_labels.append(batch[i][3])
            selected_idx.append(batch[i][4])
            cloud_ind.append(batch[i][5])
            idx.append(find_map_2d(batch[i][0], batch[i][2]))
        ...
        random_pc = np.stack(random_pc)
        random_labels = np.stack(random_labels)
        idx = np.stack(idx)

        flat_inputs, random_flat_inputs = self.tf_map(selected_pc, selected_labels, random_pc, random_labels, selected_idx, cloud_ind)
        ...
        random_inputs = {}
        random_inputs['xyz'] = []
        for tmp in random_flat_inputs[:num_layers]:
            random_inputs['xyz'].append(torch.from_numpy(tmp).float())
        random_inputs['neigh_idx'] = []
        for tmp in random_flat_inputs[num_layers: 2 * num_layers]:
            random_inputs['neigh_idx'].append(torch.from_numpy(tmp).long())
        random_inputs['sub_idx'] = []
        for tmp in random_flat_inputs[2 * num_layers:3 * num_layers]:
            random_inputs['sub_idx'].append(torch.from_numpy(tmp).long())
        random_inputs['interp_idx'] = []
        for tmp in random_flat_inputs[3 * num_layers:4 * num_layers]:
            random_inputs['interp_idx'].append(torch.from_numpy(tmp).long())
        random_inputs['features'] = torch.from_numpy(random_flat_inputs[4 * num_layers]).transpose(1, 2).float()
        random_inputs['labels'] = torch.from_numpy(random_flat_inputs[4 * num_layers + 1]).long()

        idx = torch.from_numpy(idx).long()

        return inputs, random_inputs, idx

    def tf_map_baf_lac(self, batch_pc, batch_label, random_pc, random_labels, batch_pc_idx, batch_cloud_idx):
        features = batch_pc
        input_points = []
        input_neighbors = []
        input_pools = []
        ...
        batch_graph_idx_polar = DP.knn_search(input_points[0], input_points[0], cfg.k_n)
        input_list = input_points + input_neighbors + input_pools + input_up_samples
        input_list += [features, batch_label, batch_pc_idx, batch_cloud_idx]
        ...
        batch_graph_idx_random = DP.knn_search(random_input_points[0], random_input_points[0], cfg.k_n)

        random_input_list = random_input_points + random_input_neighbors + random_input_pools + random_input_up_samples
        random_input_list += [random_features, random_labels]

        return input_list, random_input_list, batch_graph_idx_polar, batch_graph_idx_random
    ...
    def collate_fn_baf_lac(self, batch):
        ...
        flat_inputs, random_flat_inputs, batch_graph_idx_polar, batch_graph_idx_random = self.tf_map_baf_lac(selected_pc, selected_labels, random_pc, random_labels,
                                                      selected_idx, cloud_ind)
        ...
        inputs['features'] = torch.from_numpy(flat_inputs[4 * num_layers]).transpose(1, 2).float()
        inputs['labels'] = torch.from_numpy(flat_inputs[4 * num_layers + 1]).long()
        inputs['graph_idx'] = torch.from_numpy(batch_graph_idx_polar).long()
        ...
        random_inputs['graph_idx'] = torch.from_numpy(batch_graph_idx_random).long()
        idx = torch.from_numpy(idx).long()
...
if __name__ == "__main__":
    ...
    tqdm_loader = tqdm(random_loader, total=len(random_loader))

    for batch_idx, (polar_data, random_data, idx) in enumerate(random_loader):
        xyz1 = random_data['features'].transpose(1, 2).cpu().numpy()
        xyz2 = polar_data['features'].transpose(1, 2).cpu().numpy()

        b1 = np.sort(xyz1, axis=1)

        idx = np.take_along_axis(xyz1, xyz1.argsort(axis=1), axis=1)
        print(idx, idx.shape)
        print(xyz1[idx])
        exit()
