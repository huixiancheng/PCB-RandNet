...
class BAF_LAC(nn.Module):
    def __init__(self, config, learn=False):
        ...
        self.attention_map = nn.ModuleList()
        self.enhance = nn.ModuleList()
        self.interp = nn.ModuleList()
        self.decoder_blocks = nn.ModuleList()

        for j in range(self.config.num_layers):
            self.high_encoder.append(pt_utils.Conv2d(self.dim[-j - 1], self.dim[-j - 2], kernel_size=(1, 1), bn=True))
            self.attention_map.append(pt_utils.Conv2d(self.dim[-j - 2], self.dim[-j - 2], kernel_size=(1, 1), bn=False, activation=None))
            self.enhance.append(pt_utils.Conv2d(self.dim[-j - 2], self.dim[-j - 2], kernel_size=(1, 1), bn=True))
            self.interp.append(pt_utils.Conv2d(self.dim[-j - 1], self.dim[-j - 2], kernel_size=(1, 1), bn=True))
            self.decoder_blocks.append(pt_utils.Conv2d(self.dim[-j - 2], self.dim[-j - 2], kernel_size=(1, 1), bn=True))

        self.fc1 = pt_utils.Conv2d(32, 32, kernel_size=(1, 1), bn=True)
        self.fc2 = pt_utils.Conv2d(32, 32, kernel_size=(1, 1), bn=True)

        self.neigh = pt_utils.Conv2d(32, 32, kernel_size=(1, 1), bn=True)
    ...
    def forward(self, end_points):
        ...
        f_decoder_list = []
        for j in range(self.config.num_layers):
            f_high_encoder = self.high_encoder[j](f_encoder_list[-j - 1])
            f_high_encoder_interp = self.nearest_interpolation(f_high_encoder, end_points['interp_idx'][-j - 1])
            attention_map = self.attention_map[j](f_high_encoder_interp)
            attention_map = self.sigmoid(attention_map)

            f_fuse = f_encoder_list[-j - 2] * attention_map + f_encoder_list[-j - 2]
            f_enhance = self.enhance[j](f_fuse)


            f_interp_i = self.nearest_interpolation(features, end_points['interp_idx'][-j - 1])
            f_interp_i = self.interp[j](f_interp_i)
            f_decoder_i = self.decoder_blocks[j](f_interp_i + f_enhance)
        ...
        f_neigh = self.gather_neighbour(features.squeeze(-1).permute((0, 2, 1)), end_points['graph_idx'])
        f_neigh = f_neigh.permute((0, 3, 1, 2))

        f_neigh = self.neigh(f_neigh)
        f_neigh = f_neigh.max(dim=3, keepdim=True)[0] + features

        features = self.dropout(f_neigh)
...
if __name__ == "__main__":
    ...
    from config import ConfigSemanticKITTI_BAAF as cfg

    train_dataset = SemanticKITTI('training')

    train_loader = DataLoader(
        train_dataset,batch_size=1,shuffle=True,
        num_workers=0,collate_fn=train_dataset.collate_fn_baf_lac,)
